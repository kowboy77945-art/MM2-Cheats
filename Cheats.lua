...
-- WindUI Window
local Window = Wind.new("SourceHub • " .. GameName, {
    Icon = "sword",
    Subtitle = "Executor: " .. executor,
    AccentColor = Color3.fromRGB(100, 150, 255),
    Theme = "Dark",
    Transparent = true,
    StartSize = UDim2.fromOffset(580, 460),
    StartPosition = UDim2.fromScale(0.5, 0.5),
    Draggable = true,
    Resizable = true,
...
-- Gun Section
local GunSection = MainTab:Section({ Title = "Gun" })
    ...
    Callback = function(Value)
        if isUseHook then
            ...
            if not env.enabledGunBot then
                if env.GunBotConnection and env.GunBotConnection.Connection then
                    env.GunBotConnection.Connection:Disconnect()
                    env.GunBotConnection.Connection = nil
                end
            end
            ...
            Window:Notification({
                Title = "Not Supported",
                Content = "Your Executor does not support this function. Please use a better one.",
                Duration = 3,
            })
        end
    end
...
-- WalkSpeed Section
local WalkSpeedSection = PlayerTab:Section({ Title = "WalkSpeed" })
...
WalkSpeedSection:Input({
    Title = "WalkSpeed (Manual)",
    Default = tostring(env.Walkspeed or 16),
...
-- JumpPower Section
local JumpSection = PlayerTab:Section({ Title = "JumpPower" })
...
JumpSection:Input({
    Title = "JumpPower (Manual)",
    Default = tostring(env.Jumppower or 50),
...
Window:Notification({
    Title = "SourceHub",
    Content = "Script Loaded Successfully! Game: " .. GameName,
    Duration = 4,
})
